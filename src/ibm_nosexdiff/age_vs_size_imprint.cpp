...
// patches with female and male breeders
struct Patch
{
...
// declare the metapopulation and a pool for dispersers
...
void init_arguments(int argc, char *argv[])
{
    ...
    diploid = atoi(argv[8]);
    type = atoi(argv[9]);
    nloci_f = atoi(argv[10]);
    nloci_m = atoi(argv[11]);
    nloci_rest = atoi(argv[12]);
...
void init_pop()
{
    ...
    total_loci = nloci_f + nloci_m + nloci_rest;
    ...
    for (size_t i = 0; i < Npatches; ++i)
    {
        ...
        {
            // initialize all the individual a loci
            for (size_t locus_i = 0; locus_i < total_loci; ++locus_i)
            {
                MetaPop[i].localsF[j].g[0].push_back(init_a);
                MetaPop[i].localsF[j].g[1].push_back(init_a);
            }

            // initialize phenotypes
            MetaPop[i].localsF[j].phen = init_a;
        ...
        for (size_t j = 0; j < Nmp; ++j)
        {
            ...
            for (size_t locus_i = 0; locus_i < total_loci; ++locus_i)
            {
                MetaPop[i].localsM[j].g[0].push_back(init_a);
                MetaPop[i].localsM[j].g[1].push_back(init_a);
            }

            MetaPop[i].localsM[j].phen = init_a;
    ...
    if (!is_female)
    {
        switch(type)
        {
            case 1: // madumnal allele
                ...
                break;
            case 2: // padumnal allele
                Kid.phen = diploid ? Kid.am[1] : Kid.am[0];
                break;
            case 3: // maternal expression
