...
# nf and nm all combinations
nf.nm.comb <- as.data.frame(table(sim.data[,c("nfp","nmp")]))

for (i in 1:nrow(nf.nm.comb))
{
    if (nf.nm.comb[i,"Freq"] == 0)
    {
        next
    }

    nfp.i <- as.numeric(as.character(nf.nm.comb[i,"nfp"]))
    nmp.i <- as.numeric(as.character(nf.nm.comb[i,"nmp"]))

    print(nfp.i)
    print(nmp.i)

init.plot(filename=paste("analytical_sims_l0.5_nf_",nfp.i,"_nm_",nmp.i,sep=""),
                    width=1200,
...
pushViewport(viewport(layout=lo))
    ...
    block(row=2,col=2,
            ...
            ylim=c(-0.1,6),        
            params=list(l=0.5, nf=nfp.i, nm=nmp.i, μ=1.0, k=1.0/3),
    ...
    #    block(row=2,col=4,
    #            print.xlabels=T,
    #            ind.label=expression(paste("B, local male mating only, ",italic(ℓ),"=0.5")),
    #            ylim=c(-0.1,3),        
    #            params=list(l=0.5, nf=nfp.i, nm=nmp.i, μ=1.0, k=1.0/3),
    #            af=F,
