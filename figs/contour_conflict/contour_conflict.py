...
data = pd.read_csv("../battleground_data_vary_df_nf.csv"
        ,sep=";")
...
the_fig = multipanel.MultiPanel(
        panel_widths=[1,1]
        ,panel_heights=[1,1]
        ,filename="effect_philopatry_df_nf_l_" + gensys + ".pdf"
        ,width=10
        ,height=10
        )
...
for col_i, l in enumerate(lvals):
    ...
    # in order to calculate difference between af_mi and af_pi
    # we need to generate a pivot table
